...
interface Order {
  ...
  status: string; // akan diisi dari payment_status agar sinkron
  total_price: string;
  created_at: string;
  items: OrderItem[];
}
...
export default function CartViewPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // Gunakan payment_status sebagai status utama
        const updatedOrders = (res.data.orders || []).map((order: any) => ({
          ...order,
      ...
      <div className={`p-5 space-y-6 ${hkGrotesk.className}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-lg shadow text-center">
            <p className="text-md font-medium text-gray-800">Total Orders</p>
          ...
          <div className="bg-white p-4 rounded-lg shadow text-center">
            <p className="text-md font-medium text-gray-800">Paid</p>
            <p className="text-xl font-bold text-green-600">
              {summary.paid}
            </p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow text-center">
            <p className="text-md font-medium text-gray-800">Settlement</p>
  ...
  {filteredOrders.map((order, idx) => {
    ...
    return (
      <tr key={idx} className="border-b border-slate-400 hover:bg-gray-50 transition">
        <td className="px-4 py-3 font-medium">{order.order_code}</td>
        <td className="px-4 py-3">
          <p className="font-semibold">{order.billing_name}</p>
          <p className="text-xs text-gray-800 italic">{paketTitle}</p>
        </td>

        {/* Quantity sekarang ambil dari item pertama */}
        <td className="px-4 py-3 font-semibold text-center">{quantity}</td>
        ...
        {/* ✅ Status (sinkron dari payment_status) */}
        <td
    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[80vh]">
          ...
          {/* Total & Tombol Tutup */}
          <div className="flex flex-col sm:flex-row justify-between items-center pt-4 border-t border-slate-400">
            <div className="text-gray-800 font-semibold mb-5 sm:mb-0">
              Total Pesanan:{" "}
              <span className="text-cyan-800">
                Rp{" "}
                {parseInt(selectedOrder.total_price).toLocaleString("id-ID")}
              </span>
            </div>
            <button
              onClick={() => setSelectedOrder(null)}
